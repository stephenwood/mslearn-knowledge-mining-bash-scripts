...
# Generate the semi-random suffix for the account-names we'll use for the storage account, cognitive services, and search services.
...
az storage account create --name $storage_account_name --subscription $subscription_id --resource-group $resource_group --location $location --sku Standard_LRS --encryption-services blob --default-action Allow --allow-blob-public-access true --output none
...
ai_cognitivie_services_name="ai102cog$unique_id"

echo "Creating Azure AI service account...."
az cognitiveservices account create --kind CognitiveServices --location $location --name $ai_cognitivie_services_name --sku S0 --subscription $subscription_id --resource-group $resource_group --yes --output none
...
az search service create --name $search_service_name --subscription $subscription_id --resource-group $resource_group --location $location --sku basic --output none
...
echo "----"
echo "Azure AI Services account: ${ai_services_account_name}."
az cognitiveservices account keys list --subscription $subscription_id --resource-group $resource_group --name $ai_cognitivie_services_name
echo ----
